# permanent
## Что это?
permanent - это программа для расчёта [перманента](https://ru.wikipedia.org/wiki/%D0%9F%D0%B5%D1%80%D0%BC%D0%B0%D0%BD%D0%B5%D0%BD%D1%82 "Перманент") матрицы наивным алгоритмом по определению со сложностью **O(n!)**. Однако элементами матрицы могут быть не только обычные числа, но и многочлены от произвольного числа переменных.
### Что мы понимаем под многочленом
Под многочленом мы понимаем арифметическое выражение, записанное с использованием знаков `+`, `-`, `*` и `^` с операторами-многочленами от переменных и скобок (второй член в операторе `^` должен быть целым положительным числом, при этом первый таким быть не должен), не содержащее пробелов. Переменной считается любая последовательность символов, не содержащая знаков арифметических выражений и содержащая хотя бы один символ, отличный от `0...9` и точки. Всякая переменная, как и всякое число, считается многочленом.
Знак умножения не может быть опущен: так, `1.5x` - неверная конструкция, программа воспримет это как единую переменную. Правильно: `1.5*x`.
## Как это использовать?
Программа читает из файла `input.txt`, находящегося в той же директории, что и сама программа, и записывает результат в файл `output.txt`. В файле `input.txt` в первой строке необходимо записать число `n` - размер матрицы, у которой считается перманент (при `n > 8` программа работает слишком долго). Далее должны следовать `n` строк, содержащие `n` многочленов, разделённых пробелами. Вывод состоит из одного многочлена - перманента введённой матрицы.
Например:
~~~
\\input.txt
3
(semen+svyat)^2 x 1
(semen-svyat)^2 0 1
1*ivan_kazmenko 1 0
\\output.txt
2*svyat^2+2*semen^2+x*ivan_kazmenko
~~~
## И зачем это вообще нужно?
Я не знаю :)